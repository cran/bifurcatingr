...
#' @param B number of bootstrap samples (replicates) used in first round of
#'   bootstrapping
...
bfa_boot2fast_ls <- function(z, p, burn = 5, B){
  #step 1
  boot1 <- bfa_boot1_ls(z, p, burn = burn, B, boot_est=TRUE, boot_data=TRUE)
  #step 2
  boot2 <- apply(boot1$boot_data, 1, bfa_boot1_ls, p, burn = burn, B=1, boot_est=TRUE)
