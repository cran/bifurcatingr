...
#' @param ar_coef a vector of length p giving the autoregressive coefficients in
...
#' @return A numeric vector representing a bifurcating autoregressive (BAR) tree
...
bfa_tree_gen <- function(n, p, s1, s2, r1, r2, g, intercept, ar_coef, dist="cnorm", a){
  if (sum(ar_coef)==1) {stop("Error: the sum of the coefficients must not equal 1")}
  if (dist=="cnorm"){
