...
#' @return \item{boot_est}{a matrix containing the bootstrapped least squares
#'   estimates of the autoregressive coefficients} \item{boot_data}{a matrix
...
bfa_boot1_ls <- function(z, p, burn = 5, B, boot_est=TRUE, boot_data=FALSE){  # burn can be determined by simulation
  n <- length(z)
  ...
  out <- list()
  if(boot_est)
    out$boot_est=matrix(unlist(apply(boot_dat, 1, bfa_ls, p=p, error_cor=FALSE)),nrow=B, byrow=TRUE)
  if(boot_data)
